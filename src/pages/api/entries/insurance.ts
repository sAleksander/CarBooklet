...
export const PATCH: APIRoute = async (context) => {
  ...
  const result = insuranceEntryPatchSchema.safeParse(body);
  if (!result.success) {
    return Response.json({ error: result.error.issues[0].message }, { status: 400 });
  }
  ...
  try {
    const { id, conducted_at, mileage, insurer, policy_start_date, renewal_date } = result.data;
    const entry = await updateInsuranceEntry(supabase, id, context.locals.user.id, {
      conducted_at,
      mileage: mileage ?? null,
      insurer,
      policy_start_date,
